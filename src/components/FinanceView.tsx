  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-2xl overflow-hidden relative group">
          <div className="absolute -right-4 -top-4 w-24 h-24 bg-emerald-50 rounded-full blur-3xl group-hover:bg-emerald-500/20 transition-all"></div>
          <div className="flex items-center gap-3 text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-4">
            <TrendingUp size={14} /> Estimated Daily Margin
          </div>
          <p className="text-3xl font-mono font-bold text-gray-800 tracking-tighter">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-2xl relative group">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-2xl relative group">
          <div className="flex items-center gap-3 text-[10px] font-black text-amber-600 uppercase tracking-widest mb-4">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center">
            <div className="text-2xl font-mono font-bold text-emerald-600">{totalPresent}</div>
            <div className="text-[9px] text-gray-500 uppercase">Total Present Days</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-mono font-bold text-red-600">{totalAbsent}</div>
            <div className="text-[9px] text-gray-500 uppercase">Total Absent Days</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-mono font-bold text-blue-600">
              {totalPayroll.toFixed(1)}
            </div>
            <div className="text-[9px] text-gray-500 uppercase">Est. Payroll Days</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
        ...
        <div className="bg-gray-50/80 border border-gray-200 rounded-2xl p-6">
          <h3 className="text-xs font-black text-gray-800 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
          ...
          <div className="space-y-4">
            {margins.map((m) => (
              <div
                key={m.patient_id}
                className="bg-white border border-gray-200 p-4 rounded-xl flex justify-between items-center group hover:border-emerald-500/30 transition-all"
              >
                <div>
                  ...
                  <div className="flex items-center gap-2 text-[9px] text-gray-500 font-black uppercase tracking-widest mt-1">
                    <span className="text-blue-600">{formatPKR(m.daily_revenue)}/day</span>
                    <span>•</span>
                    <span className="text-amber-600">-{formatPKR(m.daily_cost)} cost</span>
                  </div>
                </div>
                <div className="text-right">
                  <p
                    className={cn(
                      ...
                      m.daily_margin > 0 ? 'text-emerald-600' : 'text-red-600'
                    )}
                  >
                    {m.daily_margin > 0 ? '+' : ''}
                    {Math.round(m.daily_margin).toLocaleString()}
                  </p>
                  <p className="text-[8px] text-gray-500 uppercase font-black tracking-[0.2em]">
                    Daily Net
                  </p>
                </div>
              </div>
        ...
        <div className="bg-gray-50/80 border border-gray-200 rounded-2xl p-6">
          ...
          <div className="space-y-4">
            {accruals.map((a) => (
              <div
                key={a.employee_id}
                className="bg-white border border-gray-200 p-4 rounded-xl flex justify-between items-center group hover:border-blue-500/30 transition-all"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center font-bold text-xs text-blue-600 border border-gray-200">
                    {a.full_name[0]}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-gray-800 uppercase tracking-tight">
                ...
                <div className="text-right">
                  ...
                  <div className="flex flex-col items-end">
                    <p className="text-[8px] text-blue-600 uppercase font-black tracking-[0.2em]">
                      Net Accrued PKR
                    </p>
                    {Number(a.total_penalties) > 0 && (
                      <p className="text-[8px] text-red-600 font-bold uppercase tracking-widest mt-1">
                        -{Number(a.total_penalties).toLocaleString()} Penalty
                      </p>
                    )}
                    {Number(a.total_advances) > 0 && (
                      <p className="text-[8px] text-amber-600 font-bold uppercase tracking-widest mt-1">
